...
2026-09-13 の初回の自動実行では、`watch` が67ページを巡回して14件の変化を検知し Issue を立て、
`scan` は変化なしで Issue を立てなかった。意図どおりに動いている。

**Issue が来たら**: 「内容が変わった可能性がある」欄の自治体だけ出典ページを開き直し、
変わっていれば JSON と `lastVerified` を更新する。「本文が変わったが主要項目に変化なし」は
お知らせ欄の更新なので基本は放置でよい。
...
### GitHub Actions からは取れないサイトがある

手元からは取得できるのに、CI のランナーからは弾かれるサイトがある（データセンターIPの
ブロックと思われる）。2026-09-13 の巡回では 港区・岡山市・相模原市・練馬区（ネリサポ）が
これに当たった。**回避策は入れない。** Issue の「取得できなかった」欄に残るので、
その自治体は手元で `npm run screen` を流して確認する。
